...
farm = os.environ.get("FARM", "Farm2")
table = os.environ.get("TABLE", "whales")
...
try:
    while True:
        ...
        print('Whale location sent..')
        whale_id = random.randint(1, 35)
        longitude = random.uniform(-180, 180)
        latitude = random.uniform(-90, 90)
        date_time = datetime.now()
        file_path = f"./image/{whale_id}.png"

        # SQL query to insert the sheep location data
        QUERY = f"INSERT INTO {table} (farm, whale_id, longitude, latitude, date_time, file_path) \
            VALUES (%s, %s, %s, %s, %s, %s)"
        values = (farm, whale_id, longitude, latitude, date_time, file_path)
