...
farm = os.environ.get("FARM", "Farm3")
table = os.environ.get("TABLE", "birds")
...
try:
    while True:
        ...
        print('Bird location sent..')
        bird_id = random.randint(1, 35)
        longitude = random.uniform(-180, 180)
        latitude = random.uniform(-90, 90)
        date_time = datetime.now()
        file_path = f"./image/{bird_id}.png"

        # SQL query to insert the sheep location data
        QUERY = f"INSERT INTO {table} (farm, bird_id, longitude, latitude, date_time, file_path) \
            VALUES (%s, %s, %s, %s, %s, %s)"
        values = (farm, bird_id, longitude, latitude, date_time, file_path)
